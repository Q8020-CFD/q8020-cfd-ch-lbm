...
- burgers_solver - main entry point, exposes numerous knobs for both the CH and LBM routines. Uses the q8020-cfd-metautil library to pull in some common QC arguments (e.g. "--shots").
...
- lib_fw - q8020-cfd-metautil exposes a "solverfw" - a "solver framework" which this code tries to adapt to. The idea is to create pluggable components for a reusable solver scaffolding. This framework is a work-in-progress. Mostly this means providing classes which implement the solverfw abstract classes - wrappers on existing python methods in this code. [Note: solverfw is an active work in progress, what's implemented here should not be taken as end state.]
