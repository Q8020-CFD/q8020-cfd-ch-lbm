...
The shots budget is used to accumulate samples for post-selecting on the ancilla. A larger per-step damping — the price of a larger dt — lowers the success rate and throws away more shots. A small dt means deep circuits. 

Note these ancilla qubits are not the same as those used for state preparation, and are also distinct from the qubits used to represent the spatial grid - the sum is the qubit budget for the method.

The Cole-Hopf MPS encoding steps requires ancilla qubits above the q qubits we allocated for the space discretization. Additionally, the time evolution requires additional ancilla. We bound the number of total qubits the code may use as "max-total-qubits", and allow it to grow to O(C·q), under the observation that today's QC are relatively wide, but still inhibited by depth - allowing an algorithm to grow its number of utilized qubits is not of much benefit if the circuits which use them, and which are dependent on q, cannot be usefully deep.

Finally, there is the phi modes filter, a low pass filter on φ which is performed classically and has the effect of smoothing the noisy signal prior to the reverse Cole-Hopf transform, which is u = −2ν·∂/∂x(ln φ). The derivative amplifies high-frequency measurement noise, so a low-pass before the reverse CH helps u from becoming noise dominated. The filter takes an argument which is the number of modes to keep, and can be set below the full modes count, keeping enough to cover the pertinent information in a signal given its anticipated smoothness (e.g. as a function of viscosity).
